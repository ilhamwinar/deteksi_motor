...
def play_sound():
    ...
    try:
        ...
        write_log_error(input_titik,"NOT SUCCESSED TO VOICE OUTPUT MOTOR")
        pass

    write_log(input_titik,"START TO SEND NOTIFICATION TO DASHBOARD")
    try:
        ws_url="https://deteksimotor-ws.jmto.co.id/send-message/"+input_titik
        r = requests.get(url = ws_url)
        write_log(input_titik,"SUCCESSED TO SEND NOTIFICATION TO DASHBOARD DENGAN HIT KE "+ws_url)
        data = r.json()
        pesan=data['status']
        write_log(input_titik,"BALIKAN DARI API "+str(pesan))
        # logging.warning(pesan)
    except:
        write_log_error(input_titik,"NOT SUCCESSED TO SEND NOTIFICATION TO DASHBOARD")
        pass
...
def post_to_dev(id_cctv,url_image,url_video,class_detection,detection_object,waktu_deteksi):
    ...
    try:
        ...
        pesan=response['code']
        write_log(input_titik,"Response Status: "+pesan)
    except :
...
if __name__ == '__main__':
        ...
        write_log(input_titik,"START PROGRAM CCTV SURVAILANCE MOTOR V2")
            ...
            if success:
                ...
                if len(results[0].boxes.cls) > 0 :
                    ...
                    for i in results[0].boxes :
                        ...
                        y_area=int(xyxy[0][3])-int(xyxy[0][1])
                        x_area=int(xyxy[0][2])-int(xyxy[0][0])
                        luas_area=y_area*x_area
                        if name_clas == 3:
                            name_clas="Motor"
                        elif name_clas == 1:
                            name_clas="Sepeda"
                        ...
                        write_log(input_titik,str(luas_area))
                    ...
                    write_log_error(input_titik, "y panjang: "+str(y_area))
                    write_log_error(input_titik, "x panjang: "+str(x_area))
                    write_log_error(input_titik, "luas: "+str(luas_area))

                    if luas_area >= 20000:
                        write_log_error(input_titik,"KOTAK KURANG DARI 20000 "+"KOTAK SEBENARNYA :"+str(luas_area))

                        # file_name = datetime.now().strftime("%d%m%Y_%H:%M:%S")
                        # dir_pict_dataset=str(cur_dir)+"/image_dataset/IMG"+"_"+input_titik+"_"+file_name+".jpg"
                        # cv2.imwrite(dir_pict_dataset, frame_kotak)

                        write_log_error(input_titik,"gambar kotak saved")
                        break
